...
const PortBundleSchema = z.object({
  portId: z.string().length(32).regex(/^[0-9a-f]{32}$/),
  version: z.literal('1.0.0'),
  org: z.literal(ORG_NAME),
  target: z.literal(BundleTarget.direct),
...
type PortBundle_1_0_0 = z.infer<typeof PortBundleSchema>;

class PortReader_1_0_0 extends PortReader {
  version: string = PORT_READER_VERSION;
  ...
  static validateBundle(bundleData: any): PortBundle {
    const parsedBundle: PortBundle_1_0_0 = PortBundleSchema.parse(bundleData);
    // PortBundle is a superset of PortBundle_1_0_0
    return parsedBundle as PortBundle;
  }
  ...
  static async accept(bundleData: PortBundle, permissions: PermissionsStrict, folderId: string): Promise<ReadPortData | null> {
    try {
      ...
      const validatedBundle = bundleData as PortBundle_1_0_0;
      //setup crypto
      ...
      console.error('Error accepting port bundle: ', error);
      return null;
    }
  }
  ...
  async use() {
    ...
    try {
      ...
      try {
        const result = await API.newDirectChatFromPort(
        ...
        console.log('Temporary error using read port: ', error);
        console.log('We are not cleaning up the read port as it is still usable');
        return;
      }
      ...
      console.log('Error using read port: ', error);
      console.log('Cleaning up the read port as it is not usable');
      await this.clean();
      if (lineId) {
...
export default PortReader_1_0_0;