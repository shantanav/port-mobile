...
import * as MessageStorage from '@utils/Storage/groupMessages';
...
export class SendReactionGroupMessage<
...
> extends SendGroupMessage<T> {
  ...
  async send(_onUpdateSuccess?: (success: boolean) => void): Promise<boolean> {
    try {
      ...
      try {
        ...
        await MessageStorage.saveGroupMessage(this.savedMessage);
  ...
  private async loadSavedMessage() {
    const savedMessage = await MessageStorage.getGroupMessage(
      this.chatId,
      this.messageId,
    );
    if (!savedMessage) {
  ...
  private async onFailure(e: any) {
    ...
    try {
      ...
      await MessageStorage.cleanDeleteGroupMessage(
  ...
  private async cleanup() {
    ...
    await MessageStorage.permanentlyDeleteGroupMessage(
      this.chatId,
      this.messageId,
    );
    return;
  ...
  private async updateReactionInfo() {
    ...
    await MessageStorage.setHasGroupReactions(
      this.chatId,
  ...
  private async updateConnectionInfo(newSendStatus: MessageStatus) {
    ...
    if (reactionData.tombstone) {
      ...
      const latestMessage = await MessageStorage.getGroupMessage(
        this.chatId,
    ...
    //Else, update connection with reaction text.
      ...
      const targetMessage = await MessageStorage.getGroupMessage(
        reactionData.chatId,
        reactionData.messageId,
      );
      const text =
