  ...
  useEffect(() => {
    (async () => {
      if (qrData !== '') {
        ...
        const match = qrData.match(/^(?:https?:\/\/)?(?:www\.)?([^\/]+)/i);
        const hostname = match ? match[1] : null;
        if (hostname && hostname.endsWith(numberlessDomain)) {
          const jsonUrlObj = urlToJson(JSON.stringify(qrData));
          const bundle = checkBundleValidity(JSON.stringify(jsonUrlObj));
          if (!bundle) {
            onError();
            setIsLoading(false);
            return;
          ...
          //replace screen with onboarding flow
