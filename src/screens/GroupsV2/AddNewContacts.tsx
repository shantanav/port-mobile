...
type Props = NativeStackScreenProps<AppStackParamList, 'AddNewContacts'>;

const AddNewContacts = ({route, navigation}: Props) => {
  ...
  useEffect(() => {
    (async () => {
      const connections = (await getDirectChats())
        .filter(connection => connection.chatId !== chatId)
