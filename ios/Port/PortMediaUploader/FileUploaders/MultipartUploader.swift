...
class MultipartUploader : IUploader {
  ...
  func chunkFileGenerator(filePath: String, partSize: Double) -> AsyncThrowingStream<Data, Error> {
    AsyncThrowingStream { continuation in
      let fileURL = URL(fileURLWithPath: filePath)
      guard let fileHandle = try? FileHandle(forReadingFrom: fileURL) else {
        continuation.finish(throwing: NSError(domain: "PortMultipartUploader", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not open file at \(filePath)"]))
        return
      }
      Task {
        do {
  ...
  func uploadChunk(
    ...
    while attempt < maxRetries {
      do {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
          ...
          throw NSError(domain: "PortMediaUploader", code: 0, userInfo: [NSLocalizedDescriptionKey: "Upload failed with non-success status"])
        }
        if let etag = (httpResponse.allHeaderFields["Etag"] as? String) {
          ...
          throw NSError(domain: "PortMediaUploader", code: 0, userInfo: [NSLocalizedDescriptionKey: "Upload succeeded but no ETag was returned"])
        }
      } catch {
    ...
    abortRequest.httpBody = try? JSONSerialization.data(withJSONObject: jsonDict)
  ...
  func upload(
    ...
    beginRequest.setValue("application/json", forHTTPHeaderField: "Content-Type")
    let beginJson = try JSONSerialization.data(withJSONObject: ["parts": numChunks])
    ...
    let multipartResponse = try JSONDecoder().decode(MultipartUploadResponseBody.self, from: beginData)
    ...
    if urls.count != numChunks {
      throw NSError(domain: "PortMediaUploader", code: 0, userInfo: [NSLocalizedDescriptionKey: "Mismatch in expected URLs and chunks"])
    }
    ...
    if successfulChunks.count == numChunks {
      ...
      let completion = MultipartUploadCompletion(uploadId: uploadId, mediaId: mediaId, parts: sortedParts)
      completeRequest.httpBody = try JSONEncoder().encode(completion)
      _ = try await URLSession.shared.data(for: completeRequest)
      return mediaId
    } else {
      throw NSError(domain: "PortMediaUploader", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to upload all chunks"])
    }
  }
}
